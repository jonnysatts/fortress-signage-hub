...
interface FloorPlanViewerHighlightProps {
  floorPlanId: string;
  imageUrl: string;
  filterStatus: string;
  zoom: number;
...
export default function FloorPlanViewerHighlight({
  floorPlanId,
  imageUrl,
  filterStatus,
  zoom,
  ...
  const renderMarker = (marker: Marker) => {
    ...
    const commonProps = {
      ...
      className: isOverdue || isHighlighted ? 'animate-pulse' : '',
      stroke: isHighlighted ? 'hsl(var(--primary))' : 'white',
